...
def sesiCal(fileLng1,fileLng2):
    sonuc=False
    try:    
        taalDurum=checkbox2_var.get()
        if taalDurum:
            playsound(fileLng2)
        else:
            playsound(fileLng1)
        ...
        if taalDurum:
            sonuc=soundMakeOneAll(sheet,pathGbFiles,"Nl",index)
        else: 
            sonuc=soundMakeOneAll(sheet,pathGbFiles,taal,index)
    if sonuc:
        if taalDurum:
            playsound(fileLng2)
        else:
...
# Checkbox'ı oluştur
checkbox = tk.Checkbutton(root, text="Kutuları gösterme", variable=checkbox_var, command=checkbox_tiklandi)
checkbox.grid(row=9, column=0, columnspan=5)
checkbox2 = tk.Checkbutton(root, text="Allein Nederlands", variable=checkbox2_var, command=checkbox_tiklandi)
checkbox2.grid(row=10, column=0, columnspan=5)

# Durumu göstermek için bir etiket oluştur
